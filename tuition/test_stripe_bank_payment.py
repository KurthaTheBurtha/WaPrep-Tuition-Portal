...
def test_stripe_bank_payment():
    """Test bank account payment using Stripe's test bank account numbers"""
    try:
        ...
        # These are Stripe's official test bank account numbers that don't require microdeposits
        test_bank_account = {
        ...
        if confirmed_payment.status == 'succeeded':
            ...
            if confirmed_payment.next_action and confirmed_payment.next_action.type == 'verify_with_microdeposits':
                print("⚠️ Microdeposits verification required (this shouldn't happen with test numbers)")
...
def test_multiple_bank_accounts():
    """Test different Stripe test bank account numbers"""
    print("\n=== Testing Different Stripe Test Bank Account Numbers ===\n")
    
    # Stripe's official test bank account numbers
    test_accounts = [
        {
            'name': 'Test Account 1',
    ...
    for i, account in enumerate(test_accounts, 1):
        ...
        try:
            # Create a simple PaymentMethod test
            payment_method = stripe.PaymentMethod.create(
                type='us_bank_account',
                us_bank_account={
            ...
            print(f"✅ Successfully created PaymentMethod for {account['name']}")
            print(f"   Account: ****{account['account_number'][-4:]}")
            print(f"   Type: {account['account_type']}")
            
            # Clean up
            payment_method.detach()
            print(f"✅ Cleaned up PaymentMethod")
            
        except stripe.error.StripeError as e:
            print(f"❌ Failed to create PaymentMethod for {account['name']}: {str(e)}")

if __name__ == "__main__":
    print("=== Stripe Bank Account Payment Test ===\n")
    
    # Test different bank account numbers
    test_multiple_bank_accounts()
    
    # Test full payment flow
    print("\n" + "="*50)
    test_stripe_bank_payment() 